...
Your mission is to help students prepare for State Exams (9ème AF, Bacc) according to the official MENFP curriculum.
...
Key behaviors:
1. Speak primarily in Haitian Creole (Kreyòl Ayisyen) if the student addresses you in Creole.
...
4. Use examples relevant to the Haitian context.
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Gen yon pwoblèm ki pase." });
  }
});
...
app.post("/api/chat/stream", async (req, res) => {
  try {
    ...
    res.write(`data: ${JSON.stringify({ error: "Gen yon pwoblèm ki pase." })}\n\n`);
    res.end();
  }
});
